...
moon test                             # 全部单测（54 个，js 目标）
moon fmt                              # 提交前必须跑
moon info                             # 确认 .mbti 变更是有意的
powershell scripts/build-example.ps1  # 构建 + 无头冒烟 40 断言
```
...
任何改动在提交前必须四绿：`moon check`、`moon test`（54）、`moon fmt`（无 diff）、冒烟 `ALL SMOKE TESTS PASSED`（40）。
