...
moon test                             # 全部单测（37 个，js 目标）
moon fmt                              # 提交前必须跑
moon info                             # 确认 .mbti 变更是有意的
powershell scripts/build-example.ps1  # 构建 + 无头冒烟 38 断言
```
...
任何改动在提交前必须四绿：`moon check`、`moon test`（37）、`moon fmt`（无 diff）、冒烟 `ALL SMOKE TESTS PASSED`（38）。
