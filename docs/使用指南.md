...
# 3) 真机/模拟器
#    打开 微信开发者工具 → 导入项目 → 目录选 myshop/（项目根目录）
...
| `ctx.replace_state(new_state)` | 全树 diff；可删除子对象字段，根 data 键按微信约束保留 | 一次大改 / 重置业务状态容器 |
...
想表达“替换业务状态容器”就用 `replace_state`，它同样走 diff。微信 `setData`
不能删除 data 根键，因此需要可删除字段时，请把它们放在同一个子对象里整体替换。
...
("onUnload", (ctx, _) => ctx.unbind_store(cart))   // 可显式退订；框架卸载时也会兜底清理
...
`RouteDef{path, params}` ｜ `Payload`（事件参数只读包装）｜ `PageCtx` ｜ `AppCtx`

`AppCtx::raw() -> JsVal` 可在 App 生命周期处理器中取得微信 App 实例，用于框架尚未
封装的底层互操作；常规业务优先使用类型化 API。
...
| `PageCtx::replace_state(Json)` | 全树 diff；根键保留，子对象字段可删除 |
...
| `set_nav_bar_color(front, bg)` | 导航栏配色（front ∈ NavFrontColor，微信仅 #ffffff/#000000） | — |
...
| `get_location(on_result~, on_fail~)` | 当前位置（gcj02，需授权）：成功 (lat, lng)，失败必达 on_fail | — |
...
40 项冒烟就是用它写的（`scripts/smoke.js`），可以直接抄。
