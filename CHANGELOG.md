...
### Added
- 新增可直接导入微信开发者工具的两页示例，覆盖页面、组件、跨页 store、路由、
  wx 能力与分享钩子。
- 新增验收指南和参考与合规说明，给出自动化、人工验收及开源参考边界。
- `AppCtx::raw()` 开放底层实例，供框架未封装能力渐进扩展。

### Changed
- CI 和本地示例脚本改为验证复制到 `miniprogram/engine/` 的实际交付产物。
- 脚手架依赖、示例版本和文档统一到 0.5.3；修正组件装配、导入目录、状态替换与
  性能单位说明。
- 单测增至 **54**，端到端冒烟断言增至 **40**；移除正常构建中的未使用声明警告。

## [0.5.2] - 2026-09-07

### Fixed（评审源码发现 #3：异常路径清理）
- 页面 onUnload / 组件 detached 的**用户逻辑与退订清理拆为独立槽**，
  JS 包装层用 `try { … } finally { 清理 }` 保证：即使用户回调抛异常，
  订阅也必然被清理；异常记录为 console.error，不再中断微信运行时。
- 处理器 / returns 钩子统一加 try/catch 错误记录（错误边界）。
- 契约回归 +1（用户 detached 抛异常 → 订阅仍为 0）。

### Tests
- 单测 → **53**（audit 契约回归 10 条）。冒烟仍 38。
...
### Fixed
- README 中指向中文文件名文档的相对链接改为 **URL 百分号编码**
  （mooncakes 渲染器不自动编码 CJK 路径导致乱码；GitHub 不受影响）。
...
### Changed（README 主页收敛）
- 移除「路线图」进度勾选清单（工具主页不陈列开发进度）；
  发展方向统一见 [RFC 0001](docs/rfc/0001-平台适配设计.md) 与 GitHub Issues。

## [0.4.1] - 2026-09-06

### Fixed（CI 修复）
- **scaffold-e2e job 通过**：新项目从 mooncakes 拉取框架前先 `moon update`
  刷新注册表索引（此前 CI 直接解析 registry 依赖失败）。
- 脚手架依赖版本对齐 @0.4.0。

### Changed（面向工具的专业化）
- README 重写为框架主页：移除竞赛语境，统一「为什么/特性/快速开始/概念/
  性能/测试/文档/路线图」结构，接入 CI 徽章。
- 参赛材料移入 `docs/hackathon/`（不再出现在对外文档结构中）。
- mooncakes 包描述打磨为工具化一句话。
